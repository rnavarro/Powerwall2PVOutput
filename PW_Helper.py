...
def getPowerwallData(powerwall_ip):
    try:
        response = urllib.urlopen('https://' + powerwall_ip + '/api/meters/aggregates')
        webz = response.read()
...
def getPowerwallSOCData(powerwall_ip):
    try:
        response = urllib.urlopen('https://' + powerwall_ip + '/api/system_status/soe')
        webz = response.read()
        soc = json.loads(webz)
        return soc
    except StandardError as e:
...
class Connection:
    ...
    def add_status(self, date, time, energy_exp=None, power_exp=None, energy_imp=None, power_imp=None, temp=None,
                   vdc=None, battery_flow=None, load_power=None, soc=None, site_power=None, load_voltage=None,
